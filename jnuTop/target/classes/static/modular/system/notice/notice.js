 ...
 * 标记通知为已读
 */
Notice.hasRead = function(){
	if(this.check()){
		var operation = function(){
            var ajax = new $ax(Feng.ctxPath + "/notice/hasRead", function (data) {
                Notice.table.refresh();
            }, function (data) {
                Feng.error("发生错误!");
            });
 ...
 * 标记通知为未读
 */
Notice.unRead = function(){
	if(this.check()){
		
		var operation = function(){
			var ajax = new $ax(Feng.ctxPath + "/notice/unRead",function(data){
				Notice.table.refresh();
			},function(data){
				Feng.error("发生错误!");
			});
